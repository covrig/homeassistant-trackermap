...
```diff
+ Displays the history of a device tracker from HASS on a Google map. Uses Google Sheets as data storage.
```
...
* Get an API key for the Google Maps API. Edit the html file: replace `*YOURAPI*` with your API key.
